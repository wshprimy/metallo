...
class UnifiedMetalloDataset(Dataset):
    ...
    def __init__(
        ...
        if mode not in ["image", "spectral", "multimodal"]:
            raise ValueError(
                f"Invalid mode: {mode}. Must be 'image', 'spectral', or 'multimodal'"
            )
        ...
        if mode == "image":
            ...
            logging.info(
                f"Multimodal dataset: {num_dos_values} DOS values, "
                f"{min_samples_per_dos} samples per DOS, "
                f"total: {self.dataset_size}"
            )

        logging.info(
            f"UnifiedMetalloDataset initialized: mode={mode}, size={self.dataset_size}"
        )
    ...
    def __getitem__(self, index: int) -> Dict[str, torch.Tensor]:
        ...
        if self.mode == "image":
            ...
            img_idx = (
                dos_idx * self.image_dataset.images_per_split + sample_idx_within_dos
            )
            spec_idx = (
                dos_idx * self.spectral_dataset.segments_per_split
                + sample_idx_within_dos
            )
