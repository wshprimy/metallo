...
class CrossModalAttention(nn.Module):
    ...
    def forward(self, query, key_value):
        ...
        # Ensure query and key_value have the same dimensionality
        assert query.size(-1) == key_value.size(
            -1
        ), "The feature dimension must match between query and key_value."

        # Concatenate query and key_value for attention calculation
        combined = torch.cat([query, key_value], dim=1)  # [B, N_q + N_kv, D]

        # Create an attention mask to prevent attending to certain positions (optional)
        attention_mask = torch.zeros(B, 1, N_q + N_kv, N_q + N_kv, device=query.device)

        # This mask ensures query positions (0:N_q) can attend to key_value positions (N_q:N_q+N_kv)
        attention_mask[:, :, :N_q, N_q:] = 0  # Allow query -> key_value
        attention_mask[:, :, :N_q, :N_q] = -10000  # Block query -> query

        # Apply the BertSelfAttention layer
        attended = self.attention(combined, attention_mask=attention_mask)[
            0
        ]  # [B, N_q + N_kv, D]

        # Extract the attended query part (first N_q positions)
...
class SpatialCrossAttention(nn.Module):
    ...
    def __init__(
        self, query_channels, kv_channels, hidden_dim, num_heads=4, dropout=0.1
    ):
        super().__init__()
        self.hidden_dim = hidden_dim
