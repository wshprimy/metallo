...
class SpectralDataProcessor:
    ...
    def __init__(self, source_dir: str = ".dataset/libs", 
    ...
    def parse_csv_file(self, csv_path: Path) -> Optional[np.ndarray]:
        ...
        try:
            ...
            if len(df) != self.spectral_points:
                logger.warning(f"File {csv_path.name} has {len(df)} points, expected {self.spectral_points}")
                return None
            ...
            if np.any(np.isnan(y_values)) or np.any(np.isinf(y_values)):
                logger.warning(f"File {csv_path.name} contains invalid values (NaN/Inf)")
                return None
    ...
    def create_spectrum_matrix(self, csv_files: List[Path]) -> Optional[np.ndarray]:
        ...
        if len(csv_files) < self.target_files:
            logger.warning(f"Only {len(csv_files)} files available, need {self.target_files}")
            return None
        
        # Initialize matrix
        spectrum_matrix = np.zeros((self.target_files, self.spectral_points), dtype=np.float32)
        successful_files = 0
        ...
        logger.info(f"Successfully processed {successful_files}/{self.target_files} files")
        
        if successful_files < self.target_files:
            logger.error(f"Too many failed files ({self.target_files - successful_files}), aborting matrix creation")
            return None
    ...
    def save_spectrum_matrix(self, matrix: np.ndarray, output_path: Path) -> bool:
        ...
        try:
            ...
            if output_path.exists():
                ...
                logger.info(f"Saved spectrum matrix: {output_path} ({file_size:.2f} MB)")
                return True
            else:
    ...
    def run_full_processing(self) -> Dict[str, bool]:
        ...
        # Summary
        logger.info(f"Processing complete: {successful_subdirs}/{total_subdirs} subdirectories successful")
    ...
    def validate_all_outputs(self) -> Dict[str, Dict]:
        ...
        for spectrum_file in self.output_dir.rglob("spectrum.npy"):
            ...
            validation_results[relative_path] = self.validate_output_matrix(spectrum_file)
...
def main():
    ...
    logger.info(f"Processing Results: {successful_processing}/{total_processing} successful")
    ...
    for output_file, validation in validation_results.items():
        if validation.get('file_exists', False):
            ...
            logger.info(f"  {shape_ok} {output_file}: {validation['shape']} ({validation['size_mb']:.2f} MB)")
        else:
            logger.info(f"  ✗ {output_file}: {validation.get('error', 'File not found')}")
